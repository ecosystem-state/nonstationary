...
era3 <- output%>%filter(spring_year >2012)%>%group_by(cluster)%>%summarise(n=n())%>%mutate(percent=n/sum(n))
era2 <- output%>%filter(spring_year >1988 & spring_year<2013)%>%group_by(cluster)%>%summarise(n=n())%>%mutate(percent=n/sum(n))
era1 <- output%>%filter(spring_year <1989& spring_year<1966)%>%group_by(cluster)%>%summarise(n=n())%>%mutate(percent=n/sum(n))
knitr::kable(output)
...
Interpreting this many of SOM prototypes is challenging, and many of the maps appear to be redundant. One approach used with SOMs is post-hoc clustering of the maps. With spring SLP, there appears to be \~ 2 groups:
