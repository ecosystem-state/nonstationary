...
```{r cumupgai, echo=FALSE, fig.height = 11, fig.width =8}
...
```{r testcov,   echo=FALSE, message = FALSE, warnings = FALSE}
...
```{r testcovmonth,   echo=FALSE, message = FALSE, warnings = FALSE}
...
figure<-function(dat){
ggplot(data = dat, aes(x = upwelling_index_sum, y = PDO,col=period)) +
...
ncc_spring<-winter%>%filter(region=="Northern CC"& season=="Spring"&period==3)
ncc_spring[which.min(ncc_spring$stand_bakun_seasonally),]
...
ggplot(data = winter, aes(x = stand_bakun_seasonally, y =seasonal_PDO,col=period, label=Year_lag)) +
    facet_wrap(.~region, ncol = 4, scales='free') +
  geom_text() +
  geom_smooth(method = "lm", se = FALSE, aes(col=as.factor(period))) +
  scale_x_continuous(name = "Upwelling (Bakun 1˚ 6-hourly)") +
...
ggplot(data = winter, aes(x = stand_bakun_seasonally, y =seasonal_NPGO,col=period, label=Year_lag)) +
    facet_wrap(.~region, ncol = 4, scales='free') +
  geom_text() +
  geom_smooth(method = "lm", se = FALSE, aes(col=as.factor(period))) +
  scale_x_continuous(name = "Upwelling (Bakun 1˚ 6-hourly)") +
