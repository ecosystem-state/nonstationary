...
violin.phe <-ggplot(ratio.up%>%filter(survey!="Upwelling")%>%
                      filter(beta_diff>-5&beta_diff<5)%>%
                      filter(Season=="Spring"&Difference2=="Era 3 / Era 2"), 
                    aes(x=region, y=beta_diff, fill=region)) +
  theme_bw() +
  facet_wrap(~survey)+
  scale_fill_manual(values=col4[3:1], name="Region")+
  geom_violin(alpha = 0.75, lwd=0.1, bounds =c(-5,5),scale='width',trim=TRUE) +
  # stat_summary(fun="q.95", colour="black", geom="line", lwd=0.75) +
  stat_summary(fun="q.80", colour="black", geom="line", lwd=0.3) +
  ...
  geom_hline(aes(yintercept=1), size=0.5,lty=2) +
  geom_hline(aes(yintercept=-1), size=0.5,lty=2) 
violin.phe
...
violin.bio <-ggplot(ratio.bio%>%filter(Difference=="2013:2023 / 1989:2012"&Season=="Spring"&lag==0&beta_diff<5&beta_diff>-5), 
                    aes(x=survey, y=beta_diff, fill=region)) +
  theme_bw() +
  scale_fill_manual(values=c(col4[3:1],col4[1]), name="Region")+
  geom_violin(alpha = 0.75, lwd=0.1, scale='width',trim=TRUE, bounds=c(-5,5)) +
  # stat_summary(fun="q.95", colour="black", geom="line", lwd=0.75) +
  stat_summary(fun="q.80", colour="black", geom="line", lwd=0.3) +
  stat_summary(fun="q.50", colour="black", geom="line", lwd=0.6)+
  coord_flip() +
...
ratio_index$survey <- factor(ratio_index$survey,
                           levels = c("CALCOFI","RREAS","N. Copepod","S. Copepod", 
                                      "Upwelling","STI","TUMI","LUSI"))
violin.bio2 <-ggplot(ratio_index%>%filter(Difference=="2013:2023 / 1989:2012" & Index!="Upwelling"&lag==0&beta_diff<5&beta_diff>-5), 
                    aes(x=survey, y=beta_diff, fill=region)) +
  theme_bw() +
  scale_fill_manual(values=c(col4[3:1],col4[1]), name="Region")+
...
pdf("Output/ViolinRatio.pdf", 11,6) 
