...
season_dat <- climate_dat %>% pivot_longer(cols=c('seasonal_ONI', 'seasonal_PDO','seasonal_NPGO', "NPI"),
                    names_to='Index',
                    values_to='values')

annual_dat <- climate_dat %>% pivot_longer(cols=c('annual_ONI', 'annual_PDO','annual_NPGO', "NPI"),
                    names_to='Index',
                    values_to='values')
...
outdat<-climate_dat%>%select(Year_lag, season,
          seasonal_PDO, seasonal_NPGO, seasonal_ONI,NPI)%>%
  distinct()%>%
  filter(Year_lag<2023)%>%
  rename(year=Year_lag, NPGO = seasonal_NPGO, PDO = seasonal_PDO, ENSO= seasonal_ONI, NPI=NPI)%>%
  dplyr::left_join(sst_out, by = 'year')%>%
  filter(season == "Winter")%>%
  select(year, period, PDO, NPGO, ENSO, NPI,cluster, var)%>%
  distinct()%>%
 pivot_longer(
  cols = c(PDO, ENSO, NPGO,NPI),
  names_to = c("Index"),
  values_to = "value")
...
winter <- climate_dat%>%filter(season=="Spring")%>%
  select(Year_lag, region, season, seasonal_mean, stand_bakun_seasonally,stand_bakun_annual,period, era.region,seasonal_PDO)%>%
    distinct()%>%
  filter(Year_lag<2023&Year_lag!=2022)
...
#| message: false
winter <- climate_dat%>%filter(season=="Spring")%>%
  select(Year_lag, region, season, seasonal_mean, stand_bakun_seasonally,stand_bakun_annual,period, era.region,NPI)%>%
    distinct()%>%
  filter(Year_lag<2023)
...
ggplot(data = winter, aes(x = stand_bakun_seasonally, y =NPI,col=period)) +
    facet_wrap(.~region, ncol = 4, scales='free') +
  geom_point(size=0.75,aes(col=period)) +
  geom_smooth(method = "lm", se = FALSE, aes(col=as.factor(period))) +
  scale_x_continuous(name = "Upwelling (Bakun 1˚ 6-hourly)") +
