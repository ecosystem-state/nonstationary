...
data <- climate_dat_cop%>%filter(season=="Summer")%>%
  ...
  select(Year_lag, seasonal_copepod_northern,seasonal_copepod_southern,
...
ratio.beta.up<-rbind(up_post%>%filter(period=='1967 - 1988')%>%add_column((up_post%>%filter(period=='2014 - 2022')%>%group_by(Index,region))$beta-
                                                                         (up_post%>%filter(period=='1989 - 2013')%>%group_by(Index,region))$beta)%>%select(region,beta,Index,`... - ...`)%>%
                                                                         rename('beta_diff'=`... - ...`)%>%
