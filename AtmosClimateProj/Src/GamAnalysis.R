...
dat.gam<-dat.lm%>%
  mutate(trend2=as.numeric(trend),
         period2=as.numeric(period),
         Index_Name2=as.numeric(as.factor(Index_Name)))


temp<-dat.lm%>%filter(season=="Spring"&
                        trend=='RREAS (CCC)'&
                        Index_Name=="NPGO")
t3<-gam(estimate ~s(Year_lag, Index_Value, m=2, k=5),
        data=temp, method="REML")
AIC(t3)

AIC(gam(estimate ~s(Year_lag, m=2, k=5)+
      s(Index_Value, m=2, k=5),
    data=temp, method="REML"))
AIC(gam(estimate ~s(Year_lag, m=2, k=5),
        data=temp, method="REML"))
plot(t3, page=3, scheme = 2)
pred<- cbind(temp_pred,
                   predict(t3, 
                           temp_pred, 
                           se.fit=TRUE, 
                           type="response"))
pred.b<-temp%>%left_join(unique(pred))

ggplot(aes(x=Year_lag, y=Index_Value, z = fit), data=pred) +
  geom_raster(aes(fill = fit)) +
  geom_contour(colour = "white") +
  scale_fill_continuous(name = "y",type = "viridis") +
  theme_minimal() +
...
temp<-data.phe.lm%>%filter(season=="Spring"&
                        region=="Southern CC"&
                        Up_Name=='stand_tumi'&
                        Index_Name=="NPH")
t3<-gam(Up_Value ~s(Year_lag, Index_Value, m=2, k=5),
        data=temp, method="REML")
AIC(t3)
plot(t3, page=3, scheme = 2)
pred<- cbind(temp_pred,
...
ggplot(aes(x=Year_lag, y=fitIndex_Value, z = fit), data=pred) +
  geom_raster(aes(fill = fit)) +
  geom_contour(colour = "white") +
  scale_fill_continuous(name = "y",type = "viridis") +
  theme_minimal() +
