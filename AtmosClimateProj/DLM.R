...
Phen_Bio_DLM_line<-ggplot(data=Phen_Bio_DLM, aes(y = as.numeric(beta), fill=Trend,x= as.numeric(Year),col = as.factor(Trend))) +
  ...
  geom_ribbon(aes(ymin = as.numeric(beta)-as.numeric(beta_se)/2, ymax = as.numeric(beta)+as.numeric(beta_se)/2), alpha=0.1,colour = NA)+
...
pdf("Output/DLMbetawinter.pdf", 9,6) 
